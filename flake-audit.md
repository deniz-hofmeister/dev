...
| 4 | 14 LSP servers dead weight in claude closure; "Claude Code cannot drive LSP servers" | Closure facts confirmed (lemminx → headless JRE 21, marksman → dotnet-runtime 9), but the premise is outdated: Claude Code has native LSP support (~v2.0.74+) via plugins. | Inverted the fix: LSP servers stay in `claudePackages`, and `packages/claude-lsp-plugin` now generates a plugin (`.lsp.json` + manifest) mapping 16 servers to file extensions. The wrapper loads it via `--plugin-dir` and sets `ENABLE_LSP_TOOL=1`. Verified loaded: `claude plugin list` → `nix-lsp@inline ✔`. |
...
- The `--plugin-dir` plugin is session-only by design; it never touches
  `~/.claude` state. The user-scope marketplace plugins `clangd-lsp`,
  `pyright-lsp`, `rust-analyzer-lsp` overlap with `nix-lsp` for .c/.py/.rs —
  consider disabling them (`claude plugin`) to avoid double servers.
